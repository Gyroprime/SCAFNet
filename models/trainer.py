...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        self.summary_writer = SummaryWriter(f'tensorboard_log/{args.project_name}')
        self.epoch_loss = 0
        self.train_len = None
        ...
        input_tensor = torch.randn(1, 3, 256, 256).to(
            torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
        flops, params = profile(self.net_G, inputs=(input_tensor, input_tensor))
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            ...
            print("Initializing backbone weights from: " + self.pretrain)
            pretrain_weights = torch.load(self.pretrain)
            self.net_G.load_state_dict(pretrain_weights['model_G_state_dict'], strict=False)
            self.net_G.to(self.device)
            # self.net_G.eval()
        else:
    ...
    def _collect_running_batch_states(self):
        ...
        self.epoch_loss += self.G_loss.item()
    ...
    def _collect_epoch_states(self):
        ...
        self.logger.write('\n')
        if self.is_training:
            self.summary_writer.add_scalar('loss/train', self.epoch_loss / self.train_len, self.epoch_id)
            self.summary_writer.add_scalar('acc/train', self.epoch_acc, self.epoch_id)
        else:
            self.summary_writer.add_scalar('loss/val', self.epoch_loss / self.val_len, self.epoch_id)
            self.summary_writer.add_scalar('acc/val', self.epoch_acc, self.epoch_id)
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            self.val_len = len(self.dataloaders['val'])
            self.train_len = total
            self.logger.write('lr: %0.7f\n' % self.optimizer_G.param_groups[0]['lr'])
            for self.batch_id, batch in tqdm(enumerate(self.dataloaders['train'], 0),total=total,ncols=60):
